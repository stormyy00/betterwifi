...
const Navigation = () => {
  ...
  return (
    <div className="px-2 md:px-4 sticky py-3 top-0 z-30 bg-blue-400 w-screen flex justify-between items-center text-2xl sm:text-2xl md:text-xl lg:text-2xl">
      <Link
        onClick={() => {
          setSelected("");
        }}
      ...
      >
        ...
        /> */}
        <span className="sm:text-2xl md:text-xl lg:text-3xl text-white whitespace-nowrap m0-3">
          BETter wifi 
        </span>
      </Link>
      <div className="hidden md:flex justify-end w-full lg:gap-2">
        {ITEMS.map((item, index) => (
          <div key={index} className="relative">
            <Link
              ...
              className={`hover:text-art-pink-100 text-2xl lg:text2xl duration-300 border-solid font-semibold py-2 mx-2 ${
                selected === item.name
                  ? item.name === "JOIN"
